  ...
  {:else if type === 'list'}
    {#if ordered}
      <svelte:component this={renderers.list} {ordered} {...$$restProps}>
        {#each $$restProps.items as item}
          <svelte:component
            this={renderers.orderedlistitem || renderers.listitem}
            {...item}
          >
            {#each item.tokens as token}
              {#if item.loose && (token.type === 'text' || token.type === 'paragraph')}
                <p><svelte:self {...token} {renderers} /></p>
              {:else}
                <svelte:self {...token} {renderers} />
              {/if}
            {/each}
    ...
    {:else}
      <svelte:component this={renderers.list} {ordered} {...$$restProps}>
        {#each $$restProps.items as item}
          <svelte:component
            this={renderers.unorderedlistitem || renderers.listitem}
            {...item}
          >
            {#each item.tokens as token}
              {#if item.loose && (token.type === 'text' || token.type === 'paragraph')}
